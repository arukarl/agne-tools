...
function parseEstonianDate(dateStr) {
    ...
    if (day < 1 || day > 31) {
        throw new Error('Päev peab olema vahemikus 1-31');
    }
...
// Initialize person code generator
function initPersonCodeGenerator() {
    setRandomSerial();
...
function findExactMatchForZeroCO2(targetPrice, minDate) {
    ...
    if (totalTax === target) {
        return {
            ...
            note: 'Iga CO2 väärtus <= 117 g/km sobib selle maksusumma jaoks'
        };
    }
...
async function validateWithAPI(co2Wltp, estRegDate) {
    try {
        ...
        if (apiTotalTax === undefined) {
            return {error: "API vastuses puudub 'totalPrice' väli"};
        }

        return {apiTax: apiTotalTax};
    } catch (error) {
        return {error: `Viga API päringus: ${error.message}`};
    }
}

// Initialize CO2 tax calculator
function initCO2TaxCalculator() {
    document.getElementById('taxForm').addEventListener('submit', async function (e) {
        ...
        setTimeout(async () => {
            ...
            if (!result) {
                contentDiv.innerHTML = '<div style="color: #e74c3c;">Täpset vastet ei leitud. Proovi teist maksusummat.</div>';
                return;
            }

            let html = `
                <strong>CO2 WLTP:</strong> ${result.co2Wltp.toFixed(2)} g/km<br>
                <strong>Registreerimiskuupäev:</strong> ${formatDate(result.estRegDate)}<br>
            ...
            if (apiResult.error) {
                html += `<br><br><span style="color: #f39c12;">⚠️ API valideerimise viga: ${apiResult.error}</span>`;
            } else if (apiResult.apiTax !== undefined) {
                const match = Math.abs(apiResult.apiTax - result.calculatedTax) < 0.01;
                if (match) {
                    html += `<br><br><span style="color: #27ae60;">✓ EMTA API kinnitas: ${apiResult.apiTax.toFixed(2)} EUR</span>`;
                } else {
                    html += `<br><br><span style="color: #e74c3c;">✗ EMTA API erinevus: ${apiResult.apiTax.toFixed(2)} EUR (vahe: ${Math.abs(apiResult.apiTax - result.calculatedTax).toFixed(2)} EUR)</span>`;
                }
            }
